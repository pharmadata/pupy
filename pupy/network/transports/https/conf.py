...
class PupySSLAuthenticator(object):
    def __init__(self, role, keystr, certstr, castr, client_cert_required=False):
...
class TransportConf(Transport):
    ...
    info = "HTTP transport wrapped with SSL"
    name = "https"
    server = PupyTCPServer
    client = PupySSLClient
    stream=PupySocketStream
    client_transport=PupyHTTPClient.custom(keep_alive=True, user_agent=user_agent)
    server_transport=PupyHTTPServer.custom(verify_user_agent=user_agent)
    credentials = [
        'SSL_CA_CERT',
